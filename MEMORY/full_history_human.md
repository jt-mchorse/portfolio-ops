...
## 2026-09-10 (night) — 11 merges, 12 issues, all 12 code repos, zero empty hunts

**Phase A.** Eleven ready PRs, all green, all in distinct repos, all merged
squash. The silent-rot audit came back 12 clean with the one known finding —
`trending-daily`'s stale schedule, the JT-gated ops#56/#17 secret cluster.

**The loop.** Twelve issues filed and closed, one per code repo, in 134 minutes
of a 360-minute cap. I stopped on complete coverage rather than on the clock:
every code repo now has exactly one ready, green, mergeable PR and a clean
working tree with nothing unpushed, which is the state that makes the next
Phase A trivial.

**The run changed shape halfway, and that is the thing worth remembering.** The
first seven findings came from the usual code axes — a guard on one arm of a
ternary, an invariant that hides the bug it should catch, an unguarded input
domain inherited by a refactor, identity-versus-value validation. Then those
axes started thinning, and five of the last six came from somewhere else
entirely: published artifacts, generated prose, and tooling registration.

The most productive single method was regenerating an artifact and *reading the
values*. `vector-search-at-scale`'s cost table regenerated byte-identically —
which was the check I set out to run, and it passed — while its `$/query`
column read `$0.000000` on all nine rows, because a 1.7e-8 quantity was
formatted with six decimals. A reproducible artifact is not a correct one, and a
regeneration diff can never say so.

Its twin: a guard on the *computation* says nothing about the *presentation*.
That repo's own test docstring says its guard exists because a bad check "would
yield a fabricated `$0.00/query`" — the repo names that string as the defect,
and the renderer printed it for every real value.

`python-async-llm-pipelines` was the sharpest version. Its README has a
paragraph titled "Honest framing on the numbers" that gets the physics exactly
right — a pure `asyncio.sleep` has no per-request cost, so it is the *ceiling*
for fan-out — and two generators said real-API speedups "will widen because real
API I/O has more headroom". The performance repo's headline claim, inverted, in
the committed artifact *and* in the 60-second demo, with nothing in `tests/`
touching either. When a repo has a paragraph explaining a subtlety, grep every
other place that subtlety is mentioned.

And `ai-app-integration-tests` had two README locks — one pinning quoted paths,
one pinning the decision range — while the test count in its Benchmarks section
drifted from 49 to 503. Paths get locked. Ranges get locked. Numbers, and the
*directions* of claims, usually nobody.

**Five corrections to my own work, every one caught by an arm I had written.** I
put "an identity test would refuse five subclasses" in a comment before building
it; it refuses one. I guessed 11 statements in a lock where the answer is 8. My
first `ts` guard used `Number.isSafeInteger` alone, and `MAX_SAFE_INTEGER` is a
safe integer *outside* `Date`'s range, so the thing it was guarding still threw.
I added a shared predicate to one seam and left the other's inline copy — caught
by the arm asserting how many callers the predicate has. And one lock nearly
asserted a convention six of fourteen files don't follow. Write the number after
running the neighbour, not before.

**Open for JT.** One new decision issue: `python-async-llm-pipelines#109`, on
whether benchmark provenance should be persisted into the JSON or made a
required argument — found because regenerating a doc to fix prose silently
re-attributed a four-month-old measurement to this machine. Everything else
gated stays gated and untouched.

## 2026-09-11 (night) — 12 merges, 11 issues, all 12 code repos, one dominant method

**Phase A.** Twelve ready PRs, one per code repo, all green and mergeable, all squashed
and merged. The silent-rot audit was clean on all twelve repos and reported only the
known JT-gated `trending-daily` finding in `portfolio-ops`; re-running it at the close
gave the same answer, so nothing this session introduced rot.

**The method that paid, four times over: run the command the artifact itself
documents.** `chunking-strategies-lab` had a snapshot test whose own failure message
told you how to regenerate — and that command wrote a timestamped file instead, left
the committed one untouched, and turned five sibling tests red.
`embedding-model-shootout` had a `docs/benchmarks.md` whose opening paragraph says it
is regenerated by a named command; running it took the file from 44 lines to 3 and
deleted the sentence that encodes the portfolio's no-fabricated-benchmarks rule — with
all 873 tests still passing, because the lock on that artifact checks *containment*, and
a file truncated to the table still contains the table. `prompt-regression-suite` had two
README tables of tool output and a lock that reached neither; rewriting both cosines to
0.900 left 683 tests green. `python-async-llm-pipelines` claimed an invariant holds
"regardless of n" in a table whose every row had the same n — and the benchmark had no
n axis at all, so the claim could never have been shown.

**The recurring structural lesson.** A predicate or a case table with no test of where
it is *called* can be orphaned by the next edit. That bit three times in one session,
twice in my own new tests: in `mcp-server-cookbook` my synthetic-tree arm stayed green
when the call site went flat, and in `python-async-llm-pipelines` reverting the test to
a single point while leaving the case table in place turned nothing red at all. Both
now have call-site arms.

**Six corrections to my own work**, every one surfaced by a falsification arm or a
lock rather than by review. The one worth naming: I wrote `1144 passed` into an issue
body when the measured number was 683. A number nobody measured is exactly what this
portfolio exists not to ship, and prose is not an exemption — corrected in place with a
note on the issue.

**One process failure.** I committed `agent-orchestration-platform#143` directly to
`main` and pushed, skipping the session branch. I recovered without rewriting history:
`main` carries a revert whose tree is byte-identical to the pre-session commit and whose
CI is green, and the work was re-applied on a branch cut from the post-revert `main`.
Worth recording that the obvious first attempt — branching at the already-pushed commits
— produces a branch that is an *ancestor* of `main`, so its PR diff is empty.

**State at close.** Eleven ready, green, mergeable PRs, one per code repo except
`vector-search-at-scale`, which got a filed decision-revisit instead because the defect
there is a documented deliberate choice and not mine to overrule. Two decisions recorded
