...
Spec / origin: this repo's #42 (final hop in the concurrency-lock propagation arc; this is the audit source-of-truth repo).
"""
...
def test_concurrency_group_is_nonempty_string(
    workflow: str, parsed: dict[str, Any]
) -> None:
    concurrency = parsed.get("concurrency")
...
def test_concurrency_cancel_in_progress_is_true_bool(
    workflow: str, parsed: dict[str, Any]
) -> None:
    concurrency = parsed.get("concurrency")
    if not isinstance(concurrency, dict):
