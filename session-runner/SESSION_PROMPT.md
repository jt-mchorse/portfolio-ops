...
4. **Silent-rot audit pass** (observational, non-blocking). Run `scripts/audit_phase_a.py` over the same 13 repos to surface seven silent-rot fingerprints (paired-failure, stuck-registration, stale-schedule, phantom-ci, missing-timeout, missing-concurrency, unpinned-lint-config):
   ```
